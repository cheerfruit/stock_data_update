...
def get_stock_history_1m_data(codes):
    ...
    HFS_parameters = 'interval:1'
    startdate = '2015-03-19'
    enddate = '2023-06-21'
    dataformat = 'format:dataframe'
    data = THS_HQ(codes, indicators, HFS_parameters,startdate,enddate,dataformat)
    print(data)
    data.data.to_csv('~/test_thsdata', sep='\t')
    # return data
...
def main():
    ...
    # 登录函数
    thslogindemo()
    get_stock_history_1m_data('600004.SH')
    # 通过数据池的板块成分函数和基础数据函数，提取沪深300的全部股票在2020-11-16日的日不复权收盘价
